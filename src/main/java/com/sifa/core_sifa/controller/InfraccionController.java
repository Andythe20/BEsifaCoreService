...
public class InfraccionController {
    ...
    @ApiResponse(responseCode = "200", description = "Lista de infracciones obtenida exitosamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = InfraccionResponse.class)))
    ...
    @ApiResponse(responseCode = "200", description = "Infracción encontrada", content = @Content(mediaType = "application/json", schema = @Schema(implementation = InfraccionResponse.class)))
    @ApiResponse(responseCode = "401", description = "No autorizado, token no proporcionado", content = @Content())
    ...
    @ApiResponse(responseCode = "200", description = "Historial obtenido", content = @Content(mediaType = "application/json", schema = @Schema(implementation = InfraccionResponse.class)))
    @ApiResponse(responseCode = "401", description = "No autorizado, token no proporcionado", content = @Content())
    ...
    public ResponseEntity<Page<InfraccionResponse>> getInfraccionesByIdFiscalizador(
        ...
        Page<InfraccionResponse> infracciones = infraccionService.findByIdFiscalizador(idFiscalizador,
                pageable);
        return ResponseEntity.ok(infracciones);
    }
    ...
    @ApiResponse(responseCode = "200", description = "Lista de infracciones del vehículo", content = @Content(mediaType = "application/json", schema = @Schema(implementation = InfraccionResponse.class)))
    @ApiResponse(responseCode = "401", description = "No autorizado, token no proporcionado", content = @Content())
    ...
    @ApiResponse(responseCode = "201", description = "Infracción tipificada y guardada con éxito", content = @Content(mediaType = "application/json", schema = @Schema(implementation = InfraccionResponse.class)))
    ...
    public ResponseEntity<InfraccionResponse> crearInfraccion(
            @Parameter(description = "JSON de la Infracción (Objeto InfraccionCreateRequest)", required = true) @RequestPart("infraccion") @Valid InfraccionCreateRequest request,
    ...
    @ApiResponse(responseCode = "200", description = "Infracción procesada con éxito", content = @Content(mediaType = "application/json", schema = @Schema(implementation = InfraccionResponse.class)))
    @ApiResponse(responseCode = "401", description = "No autorizado, token no proporcionado", content = @Content())
    @ApiResponse(responseCode = "404", description = "Infracción no encontrada", content = @Content())
    ...
    @ApiResponse(responseCode = "200", description = "Estado modificado exitosamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = InfraccionResponse.class)))
    @ApiResponse(responseCode = "401", description = "No autorizado, token no proporcionado", content = @Content())
    @ApiResponse(responseCode = "404", description = "Infracción no encontrada", content = @Content())
    @PreAuthorize("hasAnyAuthority('USER_JPL')")
    @PatchMapping("/{id}")
    public ResponseEntity<InfraccionResponse> actualizarEstado(
            ...
            // Usamos RequestBody nativo de Swagger para inyectar un JSON de ejemplo y
        ...
        InfraccionResponse response = infraccionService.actualizarEstadoInfraccion(id, status, idUsuario, motivo);
    ...
    @ApiResponse(responseCode = "200", description = "Infracción modificada exitosamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = InfraccionResponse.class)))
    @ApiResponse(responseCode = "401", description = "No autorizado, token no proporcionado", content = @Content())
    @ApiResponse(responseCode = "404", description = "Infracción no encontrada", content = @Content())
    @PreAuthorize("hasAnyAuthority('USER_JPL')")
    @PutMapping("/{id}")
    public ResponseEntity<InfraccionResponse> editarInfraccion(
            ...
            @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Cuerpo JSON con los atributos dinámicos a modificar", required = true, content = @Content(mediaType = "application/json", schema = @Schema(example = "{\n  \"observaciones\": \"Se corrigió el detalle del parte\",\n  \"infractionCode\": 3\n}"))) @RequestBody Map<String, Object> body) {
    ...
    @Operation(summary = "Obtener coordenadas para mapa de calor", description = "Devuelve una lista plana con las coordenadas geográficas (latitud y longitud) de las infracciones. Permite filtrar por rango de fechas y por un fiscalizador específico. Ideal para renderizar mapas de calor en el Dashboard web.")
    @ApiResponse(responseCode = "200", description = "Coordenadas obtenidas exitosamente", content = @Content(mediaType = "application/json", array = @io.swagger.v3.oas.annotations.media.ArraySchema(schema = @Schema(implementation = CoordenadaDTO.class))))
    ...
    @Operation(summary = "Obtener resumen estadístico del sistema", description = "Genera un reporte consolidado que incluye el conteo total agrupado por estados (pending, accepted, rejected, exported), el Top 3 de los tipos de infracciones más frecuentes y la data de geolocalización.")
    @ApiResponse(responseCode = "200", description = "Resumen estadístico obtenido exitosamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ReporteResumenDTO.class)))
    @ApiResponse(responseCode = "400", description = "Parámetros de búsqueda inválidos (Ej: startDate mayor que endDate)", content = @Content())
    @ApiResponse(responseCode = "401", description = "No autorizado, token no proporcionado o expirado", content = @Content())
    @PreAuthorize("hasAnyAuthority('USER_APP', 'USER_ADMIN', 'USER_JPL', 'USER_SUPERVISOR')")
    ...
    @Operation(summary = "Reporte de Productividad por Fiscalizador", description = "Genera un reporte con la cantidad de infracciones cursadas por cada funcionario. Si no se envían parámetros de fecha, retorna la productividad del día en curso. Exclusivo para auditorías.")
    @ApiResponse(responseCode = "200", description = "Reporte generado con éxito", content = @Content(mediaType = "application/json", array = @io.swagger.v3.oas.annotations.media.ArraySchema(schema = @Schema(implementation = ProductividadFiscalizadorDTO.class))))
    ...
    public ResponseEntity<List<ProductividadFiscalizadorDTO>> getProductividad(
            @Parameter(description = "Fecha de inicio (YYYY-MM-DD). Opcional, por defecto es hoy.") @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate startDate,

            @Parameter(description = "Fecha de fin (YYYY-MM-DD). Opcional, por defecto es hoy.") @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate endDate) {
    ...
    @Operation(summary = "Obtener estadísticas del Dashboard", description = "Retorna un resumen liviano con el total de infracciones y la cantidad agrupada por estado (GROUP BY). "
            + "Si no se envían parámetros de fecha, filtra por el día actual (diario por defecto). "
            + "Diseñado para las tarjetas KPI del Dashboard.")
    @ApiResponse(responseCode = "200", description = "Estadísticas obtenidas exitosamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = DashboardEstadisticasDTO.class)))
    @ApiResponse(responseCode = "400", description = "Fechas inválidas", content = @Content())
    @ApiResponse(responseCode = "401", description = "No autorizado, token no proporcionado o expirado", content = @Content())
    @PreAuthorize("hasAnyAuthority('USER_APP', 'USER_ADMIN', 'USER_JPL', 'USER_SUPERVISOR')")
    @GetMapping("/estadisticas")
    public ResponseEntity<DashboardEstadisticasDTO> getEstadisticasDashboard(
            @Parameter(description = "Fecha de inicio (YYYY-MM-DD). Opcional, por defecto es hoy.") @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate startDate,
        ...
        log.info("Petición GET para estadísticas del Dashboard");

        if (startDate != null && endDate != null && startDate.isAfter(endDate)) {
            throw new IllegalArgumentException("startDate no puede ser mayor que endDate");
        }

        DashboardEstadisticasDTO estadisticas = infraccionService.obtenerEstadisticasDashboard(startDate,
                endDate, user, search);
