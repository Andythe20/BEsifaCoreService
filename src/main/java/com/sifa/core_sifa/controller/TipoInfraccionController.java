...
public class TipoInfraccionController {
        ...
        @Operation(summary = "Listar catálogo de infracciones paginado", description = "Obtiene una lista paginada de todos los tipos de infracción que se encuentran habilitados en el sistema.")
        @ApiResponse(responseCode = "200", description = "Catálogo listado correctamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = Page.class)))
        ...
        public ResponseEntity<Page<TipoInfraccionDTO>> findAll(
                ...
                log.info("Obteniendo tipos de infracción paginados - página: {}, tamaño: {}", pageable.getPageNumber(),
                                pageable.getPageSize());
                return ResponseEntity.ok(tipoInfraccionService.findAllPaged(pageable));
        }

        @Operation(summary = "Obtener detalle de un tipo de infracción", description = "Busca y retorna los datos de una tipificación específica mediante su ID.")
        @ApiResponse(responseCode = "200", description = "Tipo de infracción encontrado", content = @Content(mediaType = "application/json", schema = @Schema(implementation = TipoInfraccionDTO.class)))
        @ApiResponse(responseCode = "401", description = "No autorizado, token no proporcionado", content = @Content())
        ...
        // CREATE
        @Operation(summary = "Crear nueva tipificación de infracción", description = "Agrega una nueva normativa o tipo de infracción al catálogo base. Acción exclusiva para administradores.")
        @ApiResponse(responseCode = "200", description = "Tipo de infracción creado y habilitado exitosamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = TipoInfraccionDTO.class)))
        @ApiResponse(responseCode = "400", description = "Datos de entrada inválidos", content = @Content())
        @ApiResponse(responseCode = "401", description = "No autorizado, token no proporcionado", content = @Content())
        @PreAuthorize("hasAnyAuthority('USER_ADMIN')")
        @PostMapping
        public ResponseEntity<TipoInfraccionDTO> create(
                        @Parameter(description = "Payload con los datos de la infracción", required = true) @RequestBody TipoInfraccionDTO tipoInfraccionDTO) {

                log.info("Creando tipo de infracción");

                TipoInfraccionDTO created = tipoInfraccionService.create(tipoInfraccionDTO);

                return ResponseEntity.ok(created);
        }

        // UPDATE
        @Operation(summary = "Actualizar tipo de infracción", description = "Modifica los atributos (nombre, disposición infringida) de un tipo de infracción existente en el catálogo.")
        @ApiResponse(responseCode = "200", description = "Registro actualizado correctamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = TipoInfraccionDTO.class)))
        @ApiResponse(responseCode = "401", description = "No autorizado, token no proporcionado", content = @Content())
        ...
        // DELETE
        @Operation(summary = "Deshabilitar tipo de infracción (Soft Delete)", description = "Realiza un borrado lógico del registro cambiando su estado 'habilitado' a falso. Previene que futuras multas utilicen este código sin romper la integridad referencial de las multas antiguas.")
        ...
        public ResponseEntity<Void> delete(
                ...
                // Retorna 204 No Content que es el estándar estricto de REST para métodos
                // DELETE
